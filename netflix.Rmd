...
write.csv(NFLX, "GOOGL.csv")
# Convert the index of the AMZN xts object to a date column
NFLX$date <- as.Date(index(NFLX))
NFLX$returns <- dailyReturn(NFLX)
write.csv(NFLX, "NFLX.csv", row.names = FALSE)
data <- read.csv("NFLX.csv")
names(data) <- c("Open", "High", "Low", "Close", "Volume", "Adjusted", "returns")
write.csv(data, "NFLX_new.csv", row.names = FALSE)
data$date <- as.Date(index(NFLX))
chartSeries(data)
...
head(train)
model <- lm(returns ~ Open + Close + Adjusted + High, data = train)
...
svm_model <- svm(returns ~ Open + Close + Volume+ Adjusted, data = train, kernel = "radial")
