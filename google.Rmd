...
head(train)
model <- lm(returns ~ Open + Close + + High + date, data = train)
...
svm_model <- svm(returns ~ Open + Close + High, data = train, kernel = "radial")
...
# Train the model
model <- NULL
rf_model <- randomForest(returns ~ Open + Close + Adjusted + High, data = train, ntree = 500)
...
dt_model <- rpart(returns ~ Open + Close+ High + Adjusted, data = train)
...
# Train the model
gbm_model <- gbm(returns ~ Open + Close + High + Adjusted, data = train, n.trees = 500, interaction.depth = 4, shrinkage = 0.01, distribution = "gaussian")
