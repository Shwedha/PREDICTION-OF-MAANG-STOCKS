...
AMZN$date <- as.Date(index(AMZN))
AMZN$returns <- dailyReturn(AMZN)
write.csv(AMZN, "AMZN.csv", row.names = FALSE)
data <- read.csv("AMZN.csv")
names(data) <- c("Open", "High", "Low", "Close", "Volume", "Adjusted", "returns")
write.csv(data, "AMZN_new.csv", row.names = FALSE)
data$date <- as.Date(index(AMZN))
...
abline(model)
##According to t value low can be dropped 
```
```{r}
library(caret)
...
head(train)
model <- lm(returns ~ Open + Close + Volume + Low + High + date, data = train)
...
svm_model <- svm(returns ~ Open + Close + Volume + Low + High + date, data = train, kernel = "radial")
...
# Train the model
model <- NULL
rf_model <- randomForest(returns ~ Open + Close + Volume + Low + High, data = train, ntree = 500)
...
dt_model <- rpart(returns ~ Open + Close + Volume + Low + High + Adjusted, data = train)
...
# Train the model
gbm_model <- gbm(returns ~ Open + Close + Volume + Low + High + Adjusted, data = train, n.trees = 500, interaction.depth = 4, shrinkage = 0.01, distribution = "gaussian")
