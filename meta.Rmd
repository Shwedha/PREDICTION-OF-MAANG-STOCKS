...
META$date <- as.Date(index(META))
META$returns <- dailyReturn(META)
write.csv(META, "META.csv", row.names = FALSE)
data <- read.csv("META.csv")
names(data) <- c("Open", "High", "Low", "Close", "Volume", "Adjusted", "returns")
write.csv(data, "META_new.csv", row.names = FALSE)
data$date <- as.Date(index(META))
chartSeries(data)
...
# Train the model
model <- NULL
rf_model <- randomForest(returns ~ Open + Close + Volume + Low+ High + date, data = train, ntree = 500)
...
# Train the model
gbm_model <- gbm(returns ~ Open + Close + Volume + Low+ High, data = train, n.trees = 500, interaction.depth = 4, shrinkage = 0.01, distribution = "gaussian")
